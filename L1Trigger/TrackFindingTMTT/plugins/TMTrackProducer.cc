   ...
   for (unsigned int iPhiSec = 0; iPhiSec < settings_->numPhiSectors(); iPhiSec++) {
    ...
    for (const Stub* stub: vStubs) {
     ...
     if(sector.inside( stub )){
      ...
      if(stub->barrel()){
       barrelStubs.at(stub->layerId()).push_back(stub);
      }
    ...
    for (unsigned int innerSeedLayer = 0; innerSeedLayer < 5; innerSeedLayer++){
     for (unsigned int outerSeedLayer = 0; outerSeedLayer < 6; outerSeedLayer++){
      ...
      if((innerSeedLayer == 0 && outerSeedLayer==1) ||
       ...
       for (const Stub* stub1: barrelStubs.at(innerSeedLayer)) {
        for (const Stub* stub2: barrelStubs.at(outerSeedLayer)) {

         TrackletSeed trackletSeed(stub1, stub2, iPhiSec, settings_);
    ...
    for (unsigned int innerSeedLayer = 0; innerSeedLayer < 5; innerSeedLayer++){
     for (unsigned int outerSeedLayer = 0; outerSeedLayer < 4; outerSeedLayer++){
      ...
      if( ( innerSeedLayer == 0 && outerSeedLayer == 1 ) ||
       ...
       for (const Stub* stub1: diskStubs.at(innerSeedLayer)) {
        for (const Stub* stub2: diskStubs.at(outerSeedLayer)) {
