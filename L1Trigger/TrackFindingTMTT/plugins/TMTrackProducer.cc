...
void TMTrackProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for (unsigned int iPhiSec = 0; iPhiSec < settings_->numPhiSectors(); iPhiSec++) {
    for (unsigned int iEtaReg = 0; iEtaReg < settings_->numEtaRegions(); iEtaReg++) {
      ...
      if (not settings_->hybrid()) htRphi.init(settings_, iPhiSec, iEtaReg, sector.etaMin(), sector.etaMax(), sector.phiCentre());
	...
	for (const Stub* stub: vStubs) {
      ...
      if (not settings_->hybrid()) htRphi.end(); // Calls htArrayRphi_.end() -> HTBase::end()
    }
  }
  ...
  for (unsigned int iPhiSec = 0; iPhiSec < settings_->numPhiSectors(); iPhiSec++) {
	...
	const vector<L1track3D> vecTrk3D = settings_->hybrid() ? trackletsMatched3D : get3Dtrk.trackCands3D(useRZfilt);
  ...
  if (settings_->debug() >= 1 && nEvents <= 1000) {
    ...
    if (settings_->hybrid()) {
      cout<<"Number of tracks after Tracklet pattern reco = "<<trackletsMatched3D.size()<<endl;
    } else {
